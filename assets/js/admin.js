window.addEventListener('DOMContentLoaded', (event) => {
    if (document.querySelector('#social-feed-ez-verify-token')) {
        document.querySelector('#social-feed-ez-verify-token').addEventListener('click', function (evt) {
            verifyPageAccessToken();
        });
    }
    if (document.querySelectorAll('.page-item')) {
        document.querySelectorAll('.page-item input').forEach( evt => {
            evt.addEventListener('click', function (evt) {
                setPageID(evt.target.getAttribute('id'));
            });
        });
    }
});
function setPageID(pageID){
    ...
    document.querySelector('#social_feed_ez_page_id').setAttribute('value', pageID);
}
